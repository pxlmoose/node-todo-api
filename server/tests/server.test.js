...
describe('PATCH /todos/:id', () => {
  it('should update todo', (done) => {
    ...
    .patch(`/todos/${hexID}`)
    .set('x-auth', users[0].tokens[0].token)
    .send({                 //this will change data properties
      completed: true,
      text
    })
    .expect(200)
...
describe('GET /users/me', () => {
  ...
  it('should return 401 if not authenticated', (done) => {
    ...
    .expect(401)
    .expect((res) =>{
      expect(res.body).toEqual({})  //using to equal to see if body is exactly empty object
    }).end(done)
  });
});
...
describe('POST /users/login', () => {
  it('shouldlogin user and return auth token', (done) => {
    request(app)
    .post('/users/login')
    .send({
      email: users[1].email,
